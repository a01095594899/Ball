...
public class InterfaceTest {
    ...
    public void testPolymorphism() {
        ...
        paintables.add(new Ball(new Point(0, 0), 10));
        paintables.add(new Box(new Point(0, 0), 20, 20));
    ...
    public void testCollisionActions() {
        Ball ball = new Ball(new Point(50, 50), 10);
        ball.setVelocity(new Vector2D(100, 0));
        ...
        double oldDx = ball.getVelocity().getX();
        ball.handleCollision(wall);

        // 반사 후 방향이 바뀜
        assertEquals(-oldDx, ball.getVelocity().getX(), 0.001);
    }
}
```
...
```java
// 잘못된 코드
Object obj = new Ball(new Point(0, 0), 10);
obj.move(1.0); // 컴파일 에러!
...
public class InterfaceDesignTest {
    ...
    public void testPaintableInterface() {
        ...
        Ball ball = new Ball(new Point(100, 100), 20);
        Box box = new Box(new Point(200, 200), 50, 40);
        Triangle triangle = new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330));
    ...
    public void testMovableInterface() {
        Ball ball = new Ball(new Point(100, 100), 20);
        MovableBox movableBox = new MovableBox(new Point(200, 200), 50, 40);
        Triangle triangle = new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330));
        ...
        assertEquals(50, ball.getVelocity().getX(), 0.001, "Ball getVelocity().getX() 작동하지 않음");
        assertEquals(30, ball.getVelocity().getY(), 0.001, "Ball getVelocity().getX() 작동하지 않음");

        // move 메서드 호출 확인
        double oldX = ball.getCenter().getX();
        double oldY = ball.getCenter().getY();
        ball.move(1.0);
        assertNotEquals(oldX, ball.getCetner().getX(), "Ball이 이동하지 않았습니다");
        assertNotEquals(oldY, ball.getCetner().getY(), "Ball이 이동하지 않았습니다");
    }

    @Test
    public void testCollidableInterface() {
        Ball ball = new Ball(new Point(100, 100), 20);
        Box box = new Box(new Point(150, 150), 40, 30);
...
public class MultipleInterfaceTest {
    ...
    public void testBallMultipleInterfaces() {
        Ball ball = new Ball(new Point(100, 100), 20, Color.RED);
        ...
        Box wall = new Box(ball.getCenter().add(new Vector(15, 0)), 20, 20);
    ...
    public void testBoxMultipleInterfaces() {
        Box box = new Box(new Point(100, 100), 50, 40, Color.BLUE);
    ...
    public void testMovableBoxMultipleInterfaces() {
        MovableBox movableBox = new MovableBox(new Point(100, 100), 50, 40, Color.GREEN);
    ...
    public void testTriangleMultipleInterfaces() {
        Triangle triangle = new Triangle(new Point(100, 100), new Point(100, 130), new Point(130, 100), Color.YELLOW);
        ...
        // Triangle 고유 기능 확인
        triangle.setVelocity(new Vector2D(40, 60));
        double oldX = triangle.getCenter().getX();
        double oldY = triangle.getCenter().getY();
        triangle.move(0.5);
        assertEquals(oldX + 20, triangle.getCenter().getX(), 0.001, "Triangle X 이동이 올바르지 않습니다");
        assertEquals(oldY + 30, triangle.getCenter().getY(), 0.001, "Triangle Y 이동이 올바르지 않습니다");
    }
}
```
...
public class PolymorphismSolutionTest {
    ...
    public void testPolymorphicRendering() {
        ...
        paintables.add(new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330), Color.GREEN));
        paintables.add(new MovableBox(new Point(400, 400), 60, 50, Color.YELLOW));
    ...
    public void testPolymorphicMovement() {
        ...
        movables.add(new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330), Color.GREEN));

        // 모든 객체에 동일한 속도 설정
        for (Movable movable : movables) {
            movable.setVelocity(new Vector2D(50, 30));
        }
        ...
        for (Movable movable : movables) {
            if (movable instanceof Ball) {
                oldXPositions.add(((Ball) movable).getCenter().getX());
                oldYPositions.add(((Ball) movable).getCenter().getY());
            } else if (movable instanceof MovableBox) {
                oldXPositions.add(((MovableBox) movable).getCenter().getX());
                oldYPositions.add(((MovableBox) movable).getCenter().getY());
            } else if (movable instanceof Triangle) {
                oldXPositions.add(((Triangle) movable).getCenter().getX());
                oldYPositions.add(((Triangle) movable).getCenter().Y());
            }
        }
        ...
        for (Movable movable : movables) {
            double newX, newY;
            if (movable instanceof Ball) {
                newX = ((Ball) movable).getCenter().getX();
                newY = ((Ball) movable).getCenter().getY();
            } else if (movable instanceof MovableBox) {
                newX = ((MovableBox) movable).getCenter().getX();
                newY = ((MovableBox) movable).getCenter().getY();
            } else if (movable instanceof Triangle) {
                newX = ((Triangle) movable).getCenter().getX();
                newY = ((Triangle) movable).getCenter().getY();
            } else {
                continue;
            }
    ...
    public void testPolymorphicCollision() {
        ...
        Ball ball = new Ball(new Point(100, 100), 20, Color.RED);
        Box box = new Box(new Point(150, 150), 40, 30, Color.BLUE);
        Triangle triangle = new Triangle(new Point(200, 200), new Point(225, 200), new Point(200, 225), Color.GREEN);
    ...
    public void testNoMoreClassExplosion() {
        ...
        Triangle triangle = new Triangle(new Point(200, 200), new Point(230, 200), new Point(200, 230), Color.BLUE);
...
public class CollisionActionTest {
    ...
    public void testBounceAction() {
        Ball ball = new Ball(new Point(50, 100), 15, Color.RED);
        ball.setVecocity(new Vector2D(100, 50));
        ball.setCollisionAction(CollisionAction.BOUNCE);

        Box wall = new Box(new Point(80, 90), 20, 40, Color.GRAY);
        wall.setCollisionAction(CollisionAction.BOUNCE);

        // 충돌 전 속도 저장
        double oldDx = ball.getVelocity().getX();
        double oldDy = ball.getVelocity().getY();
        ...
        // BOUNCE 액션: 속도 반전
        assertNotEquals(oldDx, ball.getVelocity().getX(), "BOUNCE 액션 후 X 속도가 변경되어야 합니다");
        // Y 속도는 수직 충돌이 아니면 변경되지 않을 수 있음
    }

    @Test
    public void testStopAction() {
        Ball ball = new Ball(new Point(100, 100), 20, Color.BLUE);
        ball.setVelocity(80, 60);
        ball.setCollisionAction(CollisionAction.STOP);

        Box obstacle = new Box(new Point(130, 130), 30, 30, Color.BROWN);
        ...
        // STOP 액션: 속도 0
        assertEquals(0, ball.getVelocity().getX(), 0.001, "STOP 액션 후 X 속도가 0이어야 합니다");
        assertEquals(0, ball.getVelocity().getY(), 0.001, "STOP 액션 후 Y 속도가 0이어야 합니다");
    }

    @Test
    public void testDestroyAction() {
        Ball ball = new Ball(new Point(100, 100), 20, Color.GREEN);
        ball.setCollisionAction(CollisionAction.DESTROY);

        Box destroyer = new Box(new Point(110, 110), 40, 40, Color.BLACK);
    ...
    public void testPassAction() {
        Ball ball = new Ball(new Point(100, 100), 20, Color.YELLOW);
        ball.setVelocity(50, 30);
        ball.setCollisionAction(CollisionAction.PASS);

        Box passThrough = new Box(new Point(110, 110), 30, 30, Color.CYAN);
        ...
        // PASS 액션: 속도 변화 없음
        assertEquals(oldDx, ball.getVelocity().getX(), 0.001, "PASS 액션 후 X 속도가 변경되지 않아야 합니다");
        assertEquals(oldDy, ball.getVelocity().getY(), 0.001, "PASS 액션 후 Y 속도가 변경되지 않아야 합니다");
    }

    @Test
    public void testCustomAction() {
        ExplodingBall explodingBall = new ExplodingBall(new Point(100, 100), 25, Color.ORANGE);
        explodingBall.setCollisionAction(CollisionAction.CUSTOM);

        Box trigger = new Box(new Point(115, 115), 20, 20, Color.RED);
    ...
    public void testDynamicActionChange() {
        Ball ball = new Ball(new Point(100, 100), 20, Color.MAGENTA);
...
public class SimpleWorldTest {
    ...
    public void testMixedObjectManagement() {
        ...
        Ball ball = new Ball(new Point(100, 100), 20, Color.RED);
        Box box = new Box(new Point(200, 200), 50, 40, Color.BLUE);
        Triangle triangle = new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330), Color.GREEN);
        MovableBox movableBox = new MovableBox(new Point(400, 400), 60, 50, Color.YELLOW);
    ...
    public void testPolymorphicUpdate() {
        ...
        MovableBox movableBox = new MovableBox(new Point(200, 200), 40, 30, Color.BLUE);
        movableBox.setVelocity(new Vector2D(-25, 35));

        Triangle triangle = new Triangle(new Point(300, 300), new Point(325, 300), new Point(300, 325), Color.GREEN);
        triangle.setVelocity(new Vector2D(40, -20));

        Box staticBox = new Box(new Point(400, 400), 50, 50, Color.GRAY); // 움직이지 않음
        ...
        double boxX = movableBox.getCenter().getX();
        double boxY = movableBox.getCenter().getY();
        double triangleX = triangle.getCenter().getX();
        double triangleY = triangle.getCenter().getY();
        double staticBoxX = staticBox.getCenter().getX();
        double staticBoxY = staticBox.getCenter().getY();

        // 업데이트 수행
        world.update(1.0);

        // Movable 객체들만 이동했는지 확인
        assertNotEquals(ballX, ball.getCenter().getX(), "Ball이 이동하지 않았습니다");
        assertNotEquals(ballY, ball.getCenter().getY(), "Ball이 이동하지 않았습니다");
        assertNotEquals(boxX, movableBox.getCenter().getX(), "MovableBox가 이동하지 않았습니다");
        assertNotEquals(boxY, movableBox.getCenter().getY(), "MovableBox가 이동하지 않았습니다");
        assertNotEquals(triangleX, triangle.getCenter().getX(), "Triangle이 이동하지 않았습니다");
        assertNotEquals(triangleY, triangle.getCenter().getY(), "Triangle이 이동하지 않았습니다");

        // 정적 Box는 이동하지 않음
        assertEquals(staticBoxX, staticBox.getCenter().getX(), 0.001, "정적 Box는 이동하면 안됩니다");
        assertEquals(staticBoxY, staticBox.getCenter().getY(), 0.001, "정적 Box는 이동하면 안됩니다");
    }
    ...
    public void testCollisionDetectionAndHandling() {
        ...
        // 충돌 전 속도 저장
        double ball1DxBefore = ball1.getVelocity().getX();
        double ball2DxBefore = ball2.getVelocity().getX();
        ...
        // 충돌 후 속도 변경 확인
        assertNotEquals(ball1DxBefore, ball1.getVelocity.getX(), "Ball1의 속도가 변경되어야 합니다");
        assertNotEquals(ball2DxBefore, ball2.getVelocity().getX(), "Ball2의 속도가 변경되어야 합니다");
    }
}
```
...
public class AdvancedFeatureTest {
    ...
    public void testBouncingTriangle() {
        BouncingTriangle triangle = new BouncingTriangle(new Point(100, 100), new Point(130, 100), new Point(100, 130), Color.PURPLE);
        triangle.setVelocity(new Vector2D(60, 40));
        triangle.setRotationSpeed(45); // 45도/초
        triangle.setCollisionAction(CollisionAction.BOUNCE);

        Box wall = new Box(new Point(140, 140), 20, 60, Color.GRAY);
    ...
    public void testExplodingBall() {
        ...
        Box trigger = new Box(new Point(120, 120), 20, 20, Color.RED);
    ...
    public void testSpecialZone() {
        ...
        Ball ball = new Ball(new Point(175, 175), 15, Color.WHITE);
        ball.setVelocity(new Vector2D(40, 30));
        ...
        assertEquals(oldDx * 2, ball.getVelocity().getX(), 0.001, "SPEED_UP 효과로 X 속도가 2배가 되어야 합니다");
        assertEquals(oldDy * 2, ball.getVelocity().getY(), 0.001, "SPEED_UP 효과로 Y 속도가 2배가 되어야 합니다");

        // SLOW_DOWN 효과 테스트
        ball.moveTo(new Point(350, 350);
        oldDx = ball.getCenter().getX();
        oldDy = ball.getCenter().getY();

        assertTrue(slowZone.isColliding(ball), "Ball이 SlowZone에 있어야 합니다");
        slowZone.applyEffect(ball);

        assertEquals(oldDx * 0.5, ball.getVelocity().getX(), 0.001, "SLOW_DOWN 효과로 X 속도가 절반이 되어야 합니다");
        assertEquals(oldDy * 0.5, ball.getVelocity().getY(), 0.001, "SLOW_DOWN 효과로 Y 속도가 절반이 되어야 합니다");

        // GRAVITY 효과 테스트
        ball.moveTo(new Point(480, 480));
        oldDy = ball.getVelocity().getY();

        assertTrue(gravityZone.isColliding(ball), "Ball이 GravityZone에 있어야 합니다");
        gravityZone.applyEffect(ball);

        assertEquals(oldDy + 10, ball.getVelocity().getY(), 0.001, "GRAVITY 효과로 Y 속도가 증가해야 합니다");
    }
    ...
    public void testMazeWorld() {
        ...
        Ball player = new Ball(new Point(100, 100), 10, Color.YELLOW); // 통로 위치
...
public class InterfaceBenefitSummaryTest {
    ...
    public void testMaintainabilityImprovement() {
        ...
        // 새로운 타입 추가 (Star 클래스)
        Star star = new Star(new Point(300, 300), 25, Color.YELLOW);
    ...
    public void testCodeReductionImprovement() {
        ...
        mixedObjects.add(new Ball(new Point(100, 100), 20, Color.RED));
        mixedObjects.add(new Box(new Point(200, 200), 50, 40, Color.BLUE));
        mixedObjects.add(new Triangle(new Point(300, 300), new Point(330, 300), new Point(300, 330), Color.GREEN));
        mixedObjects.add(new MovableBox(new Point(400, 400), 60, 50, Color.YELLOW));
        mixedObjects.add(new Star(new Point(500, 500), 25, Color.PURPLE));
    ...
    public void testExtensibilityImprovement() {
        ...
        rotatables.add(new BouncingTriangle(new Point(200, 200), new Point(230, 200), new Point(200, 230), Color.BLUE));
        rotatables.add(new Star(new Point(300, 300), 25, Color.YELLOW));
    ...
    public void testPerformanceImprovement() {
        ...
        for (int i = 0; i < 1000; i++) {
            if (i % 3 == 0) {
                movables.add(new Ball(new Point(i, i), 10, Color.RED));
            } else if (i % 3 == 1) {
                movables.add(new Triangle(new Point(i, i), new Point(i + 15, i), new Point(i, i+15), Color.BLUE));
            } else {
                movables.add(new MovableBox(new Point(i, i), 20, 20, Color.GREEN));
            }
        }
